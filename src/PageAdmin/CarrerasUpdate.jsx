...
// === helpers idénticos a tu archivo ===
const slugify = (txt = "") =>
  txt
    .toString()
    .normalize("NFD")
...
const CarrerasUpdate = () => {
  const { id } = useParams();
  const navigate = useNavigate();

  // === mismo estado/lógica que tu archivo ===
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await Config.getCarrerasShow(id);
        setUniversityId(data.university_id ?? "");
        setName(data.name ?? "");
        setSlug(data.slug ?? "");
        setLevel(data.level ?? "");
        setLevels(ensureArray(data.levels));
        setArea(data.area ?? "");
        // NUEVO: cargar división
        setDivision(data.division ?? "");
        setTermsUnit(data.terms_unit ?? "");
        setModality(data.modality ?? "");
        setDescription(data.description ?? "");
        setDurationTerms(ensureArray(data.duration_terms));
        setCurrentImageName(data.career_url || "");
      } finally {
        setLoading(false);
      }
    })();
  ...
  // Reemplaza tu función addDuration por esta
  const addDuration = (e) => {
  ...
  const submit = async (e) => {
    ...
    const payload = {
      ...
      terms_unit: terms_unit || undefined,
      modality: modality || undefined,
      description: description || undefined,
    };
    if (imageB64 && imageB64.startsWith("data:image/")) {
      payload.file = imageB64;
      payload.career_url = imageB64;
    }
    await Config.getCarrerasUpdate(payload, id); // PUT
