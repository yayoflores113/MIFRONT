...
const CursosStore = () => {
  ...
  const submit = async (e) => {
    ...
    const payload = {
      ...
      level: level || undefined, // "todos" | "principiante" | "intermedio" | "experto"
    ...
    if (imageB64 && imageB64.startsWith("data:image/")) {
      payload.card_image_base64 = imageB64;
    }

    const { data: created } = await Config.getCursosStore(payload); // POST /admin/courses
    if (imageB64 && created?.id) {
      await Config.getCursosUpdate({ file: imageB64 }, created.id);
    }
