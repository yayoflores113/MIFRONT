// PlanesUpdate.jsx — mismo diseño que el resto del admin, lógica intacta
...
const CTA_TYPES = ["trial", "subscribe", "contact"];
...
const PlanesUpdate = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await Config.getPlanesShow(id); // GET /admin/plans/{id}
        setName(data.name ?? "");
        setSubtitle(data.subtitle ?? "");
        setSlug(data.slug ?? "");
        setDescription(data.description ?? "");
        setCtaType(data.cta_type ?? "trial");
  ...
  const submit = async (e) => {
    ...
    await Config.getPlanesUpdate(payload, id); // PUT /admin/plans/{id}
