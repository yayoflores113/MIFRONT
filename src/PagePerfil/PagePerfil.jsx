...
const PagePerfil = () => {
    ...
    const loadUserData = async () => {
      try {
        ...
        setUser({
          ...
          role:
            (Array.isArray(userData.roles) && userData.roles[0]?.name) ||
            "Estudiante",
  ...
  // ========== CARGAR CURSOS COMPRADOS/SUSCRITOS ==========
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const loadTestAttempts = async () => {
      ...
      try {
        ...
        setUserStats((prev) => ({
          ...prev,
          testsCompleted: attempts.filter((a) => a.status === "completed")
            .length,
        }));
      } catch (error) {
  ...
  // ========== COMPONENTE: UNIVERSIDADES FAVORITAS ==========
            ...
            >
              <CardBody className="flex flex-row items-center justify-between p-4">
                ...
                <Button
                  ...
                  color="danger"
                  isIconOnly
                  onPress={() => handleRemoveFavorite(uni.id, "university")}
                >
                ...
                <CardBody className="p-4">
                  <div className="flex gap-3">
                    ...
                    <Button
                      ...
                      color="danger"
                      isIconOnly
                      onPress={() => handleRemoveFavorite(course.id, "course")}
                    >
                      <TrashIcon className="w-4 h-4" />
                    </Button>
                  </div>
